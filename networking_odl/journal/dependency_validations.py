...
def validate_router_operation(session, object_uuid, operation, data):
    ...
    if operation in (odl_const.ODL_CREATE, odl_const.ODL_UPDATE):
        ...
        if db.check_for_pending_or_processing_ops(session, object_uuid,
        ...
        if db.check_for_pending_remove_ops_with_parent(session, object_uuid):
            return False
...
def validate_router_interface_operation(session, object_uuid, operation, data):
    ...
    if operation == odl_const.ODL_ADD:
        ...
        if db.check_for_pending_or_processing_ops(session, data['id']):
            return False

        # TODO(rcurran): Check for port_id?
        if db.check_for_pending_or_processing_ops(session, data['subnet_id']):
            return False
    elif operation == odl_const.ODL_REMOVE:
        if db.check_for_pending_or_processing_add(session, data['id'],
                                                  data['subnet_id']):
            return False
